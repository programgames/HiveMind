...
| **`/home/hminstall.lua` n'est plus une copie mais un relais** | Deux copies complètes n'ont pas tenu : listes de fichiers différentes une fois, et une copie assez vieille pour prendre `--clean` pour un **nom de branche** une autre — dix-sept téléchargements depuis une branche `--clean`. L'installeur y écrit maintenant un relais de quinze lignes, sans liste ni options, qui transmet tout à `tools/hminstall.lua`. Rien dedans ne peut plus périmer |
